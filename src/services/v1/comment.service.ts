...
export const getRecipeComments = async (recipeId: string): Promise<any[]> => {
  try {
    ...
    const comments = await Comment.find({ recipe: recipeId })
      .populate('user', 'name email'); // Populate user's username and email
 ...
 */
export const createRecipeComment = async (userId: string, recipeId: string, commentText: string): Promise<any> => {
    try {
